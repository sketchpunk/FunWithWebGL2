...
class RenderLoop{
	constructor(callback, fps = 0, dataRef = null){
		...
		this.set_fps_limit( (fps > 0)?fps:0  );
	}
	...
	set_frame_caller(fc){ this.frameCaller = fc; return this; }
	set_fps_limit(v){
		if(v <= 0){
			this._fpsLimit = 0;
			this._runPtr = this.run_full.bind(this);
		}else{
			this._fpsLimit = 1000/v; //Calc how many milliseconds per frame in one second of time.
			this._runPtr = this.run_limit.bind(this);
		}
	}
	...
	run_limit(){
		...
		if(msDelta >= this._fpsLimit){ //Now execute frame since the time has elapsed.
			...
			if( this.info ){
				this.info.delta_time 	= deltaTime;
				this.info.since_start	= sinceStart;
			}
	...
	run_frame( dt ){
		if( this.info ){
			this.info.delta_time 	= dt;
			this.info.since_start	+= dt;
			this._callBack( dt, this.info.since_start );
		}
	}
}
